...
@Model.register("seq2seq_multihead_lambda")
class Seq2SeqMultiHeadLambda(Model):
    def __init__(
            self,
            vocab: Vocabulary,
    ...
    def forward(
        ...
        if self._use_gradient_reversal and target_tokens:
            domain_prediction_loss, ff_predictions = self._domain_prediction(state, source_lang)
            outputs['lp_loss'] = domain_prediction_loss

            # Training
            if self.training:
                if "loss" in outputs:
                    outputs['loss'] += self._loss_weights[DOM_TASK] * outputs['lp_loss']
                else:
                    outputs['loss'] = self._loss_weights[DOM_TASK] * outputs['lp_loss']
            else:
                self._lp_acc(ff_predictions, source_lang.squeeze(-1))
    ...
    def _domain_prediction(self, encoder_state: Dict[str, torch.Tensor], locale_labels: torch.Tensor):
        ...
        # Feedforward layer
        ff_output = self._domain_predictor_ff(encoder_outputs).squeeze(-2)

        return self._gradient_reverse_loss(ff_output, locale_labels.squeeze(-1)), ff_output
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        lp_metrics = {"lp_acc": self._lp_acc.get_metric(reset)}
        return {**sql_metrics, **nl_metrics, **lp_metrics}
